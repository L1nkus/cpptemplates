...
public:
    T lower_bound(const T &key) {
        auto it1 = ::lower_bound(large.begin(), large.end(), key);
        auto it2 = small.lower_bound(key);
        /* auto it2 = ::lower_bound(small.begin(), small.end(), key); */
        return it1 != large.end() && (it2 == small.end() || *it1 <= *it2) ? *it1 : it2 == small.end() ? INF : *it2;
    }
    bool insert(const T &val){
        auto it = ::lower_bound(large.begin(), large.end(), val);
        if (it != large.end() && *it == val)
            return 0;
        bool inserted = small.insert(val).second;
        if (!inserted)
            return 0;
        /* auto it2 = ::lower_bound(small.begin(), small.end(), val); */
        /* if (it2 != small.end() && *it2 == val) */
        /*     return 0; */
        /* small.insert(it2, val); */
        if (small.size() >= small_bound) {
...
// 1M z vecxsetem dla 30000 0.487s najlepiej.
int main(){
