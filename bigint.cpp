...
int main(){
    ...
    int n = 995;
    int cur = 2;
    vector<int> bigint1({1});
    vector<int> bigint2({2});
    while(n--){
        vector<int> big2orig = bigint2;
        auto it2 = bigint2.begin();
        auto it1 = bigint1.begin();
        int carry = 0;
        while(it1 < bigint1.end()){
            *it2++ = (carry += (*it1++)+*it2)%10;
            carry /= 10;
        }
        while(carry && it2 < bigint2.end()){
            *it2++ = (carry+=*it2)%10;
            carry /= 10;
        }
        while(carry){
        ...
        bigint1 = big2orig;
        for(auto i = bigint2.rbegin(); i < bigint2.rend(); ++i)
            cout << *i;
        cout.put(10);
    }
    /* while(cur < n){ */
