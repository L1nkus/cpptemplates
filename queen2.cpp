...
inline void rec(int line, bool diag1[], bool diag2[], bool row[]){
    //ans = max(ans,line);
    if(line == n){
        ++ans;
        return;
    ...
    FOR(i,0,n){
        if(!row[i] && !diag1[i+line+1] && !diag2[line+n-i]){
            row[i] = diag1[i+line+1] = diag2[line+n-i] = 1;
            rec(line+1,diag1,diag2,row);
            row[i] = diag1[i+line+1] = diag2[line+n-i] = 0;
        }
    }
}
